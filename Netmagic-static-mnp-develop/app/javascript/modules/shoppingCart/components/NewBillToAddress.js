...
var NewBillToAddress = createReactClass({
    ...
    showBillToAddresses(addressList) {
        let billToAdd = null;
        if(_.isArray(addressList) && addressList.length) {
            billToAdd = _.map(addressList, (address, key) => {
                return (
                    <li className="clearfix" key={key} onClick={this.props.updateBillAddressInformation.bind(null,address)}>
                        <div className="selectCompany">
                            <input type="radio" checked={_.isEqual(address,this.props.selectedBillAddress)}/>
                        </div>
    ...
    render() {
        let {billAddressList} = this.props;
        return (<div>
            ...
            <div className={this.getToggleClassSet()}>
                <ul className="contactListing clearfix">
                    {this.showBillToAddresses(billAddressList)}
                </ul>
            </div>
        </div>
        )
